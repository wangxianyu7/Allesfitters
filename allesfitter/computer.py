...
def flux_subfct_ellc(params, inst, companion, xx=None, settings=None, t_exp=None, n_int=None, return_fluxes=False):
    ...
    if (params[companion+'_rr'] is not None) and (params[companion+'_rr'] > 0):
        ...
        # tm.set_data(xx, lcids=lcids, exptimes=exptimes, nsamples=nsamples)
        
        if np.sum(cadence) == 0:
            tm.set_data(xx)
        elif len(cadence)!= len(xx):
            tm.set_data(xx)
        else:
            tm.set_data(xx, lcids=lcids, nsamples=nsamples, exptimes=exptimes)
        
        model_flux1 = tm.evaluate(k, ldc, t0, p, a, i, e, w)
